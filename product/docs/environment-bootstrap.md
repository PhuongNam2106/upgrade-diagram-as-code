...
4. Trong `D:\upgrade-diagram-as-code\product`, chạy `npm ci`.
5. Trong `product\deploy`, tạo `.env` từ `.env.example` và thay API key mẫu.

Tạo key ngẫu nhiên trong PowerShell:
...
Không đưa API key vào `.diagramrc.yml`, source code, workflow literal hoặc log.

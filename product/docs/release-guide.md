...
1. Xác nhận mọi package có version `0.3.0` và release notes tồn tại tại `product/docs/releases/0.3.0.md`.
...
```text
product/release/product-v0.3.0/
```
...
- `diagram-as-code-action-0.3.0.cjs`
...
```powershell
Get-FileHash .\release\product-v0.3.0\diagram-as-code-server-0.3.0.zip -Algorithm SHA256
Get-Content .\release\product-v0.3.0\SHA256SUMS
```
...
docker build -f gateway/Dockerfile -t diagram-as-code-gateway:0.3.0 .
docker image inspect diagram-as-code-gateway:0.3.0
```
...
Tag `product-v0.3.0` kích hoạt `Product Release`. Workflow publish Gateway image lên GHCR trước, sau đó kiểm tra và tạo GitHub Release chứa cả VSIX lẫn `diagram-as-code-server-0.3.0.zip`. Không tạo tag trước khi commit chuẩn bị release đã có mặt trên remote.
...
- Xác nhận Gateway image trên GHCR public có thể pull theo tag `product-v0.3.0`.
...
4. Giải nén Windows ZIP `0.3.0` rồi chạy `install`. Installer đọc `.env` đã có và giữ nguyên key.

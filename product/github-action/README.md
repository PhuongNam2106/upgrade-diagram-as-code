...
This JavaScript Action verifies that committed SVG files match their diagram sources. It checks changed files on normal pull requests and all sources when `.diagramrc.yml` or `.diagram-renderer.lock` changes.

```yaml
- uses: PhuongNam2106/upgrade-diagram-as-code/product/github-action@product-v0.1.0
  with:

...
async function render(baseUrl: string, apiKey: string, request: ReturnType<typeof deterministicRequest>): Promise<string> {
  const response = await fetch(`${baseUrl.replace(/\/$/, "")}/v1/render`, {
    method: "POST",
    headers: {
  ...
  if (!response.ok) {
    throw new Error(`Gateway render failed (${response.status}): ${body}`);
  }
  return body;
}
...
async function run(): Promise<void> {
  ...
  const configPath = core.getInput("config-path") || ".diagramrc.yml";
  ...
  const relativeSources = await fastGlob(config.sources.include, {
    cwd: path.join(root, config.sources.directory),
    onlyFiles: true,
    dot: false,
  });
  const allSources = relativeSources.map((file) => path.posix.join(config.sources.directory.replaceAll("\\", "/"), file.replaceAll("\\", "/")));
  ...
  for (const item of plan) {
    ...
    const svg = await render(gatewayUrl, apiKey, deterministicRequest(item.sourcePath, source));
    if (!existsSync(absoluteOutput)) {
      stale.push({ ...item, reason: "missing" });
    } else if (!readFileSync(absoluteOutput).equals(Buffer.from(svg, "utf8"))) {
